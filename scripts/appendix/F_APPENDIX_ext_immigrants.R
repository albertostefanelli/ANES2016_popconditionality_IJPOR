##### FIT MODEL WITH MPLUS TO GENERATE FIGURE 5 IN THE APPENDIX #####
...
data_mplus$imm3_ext <- sqrt((data_mplus[["V162269"]] - 2)^2)
...
model <- paste(
  ...
  [imm3_ext] (extb0g1);         ! free intercept
  imm3_ext (vareg1);            ! free residual

  !!! freely estimated slopes !!!
  imm3_ext ON POP (extb1g1);
  imm3_ext ON bernie (extb2g1); !fixed
  ...
  wagner_aff WITH imm3_ext (covg1);


  imm3_ext ON EFFI (exts01g1) ! internal efficacy
  ...
  [imm3_ext] (extb0g3);        ! free intercept
  imm3_ext (vareg3);           ! free residual

  !!! freely estimated slopes !!!
  imm3_ext ON POP (extb1g3);
  imm3_ext ON bernie (extb2g1);! fixed
  ...
  wagner_aff WITH imm3_ext (covg3);
...
model_out <- "F_APPENDIX_ext_immigrants.inp"
