##### READ MPLUS .OUT FILES TO GENERATE FIGURE 6 IN THE APPENDIX #####
...
mod_to_read <- "F_APPENDIX_AFF_diff_leaders.out"
...
ci_90_aff_dem_stdy_low <- all_output$parameters$ci.stdyx.standardized %>% filter(LatentClass==1)  %>% filter(str_detect(paramHeader,".ON")) %>%
  filter(str_detect(paramHeader,"LEADER_F.")) %>%
...
ggsave(here("figures", "APPENDIX_affective_diff_thermo.png"),width=990*3,height=250*3, dpi=300,  units = "px")
