#### FIT MAIN TEXT POOLED MODEL USING MPLUS ####
# Model specifications:
# coefficient of populism FIX across DEM and REP
# all other parameters are fixed across Democrats and Republicans
# covariances latent variables set to 0
# parameters for pure independents free to vary
...
data_mplus_subset <- data_mplus_centred |> select(variable_to_model_mplus)
...
model <- paste(
  ...
  [EXT] (extb0g1);         !  intercept
  EXT (vareg1);            !  residual

  !!! estimated slopes !!!
  EXT ON POP (extb1g1);
  EXT ON bernie (extb2g1);

  !! AFFECTIVE !!

  [wagner_aff] (affb0g1);  !  intercept
  wagner_aff (varag1);     !  residual
  ...
  [wagner_aff] (affb0g3); ! intercept
  wagner_aff (varag3);    ! residual
...
model_out <- "F_MAIN_fixed_pop.inp"
...
for (i in 1:length(f_rad)) { file.move(glue(here("{f_rad[,i]}")), here("scripts", "manuscript", "mplus_out"),overwrite = TRUE) }
